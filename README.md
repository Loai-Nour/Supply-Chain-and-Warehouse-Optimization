...
classDiagram
    ...
    %% The GUI inherits from Tkinter's root window
    %% SCWOS_GUI --|> tk.Tk : inherits

    %% The GUI is COMPOSED OF the backend subsystems (Strong ownership)
    SCWOS_GUI *-- InventoryManager
    SCWOS_GUI *-- Warehouse
    SCWOS_GUI *-- OptimizationEngine
    SCWOS_GUI *-- TransactionLogger
    
    %% The GUI manages a list of orders (Aggregation)
    SCWOS_GUI o-- CustomerOrder

    %% The UI Frames inherit from Tkinter Frame and use the controller
    class InventoryFrame
    class WarehouseFrame
    class OrderFrame
    class DashboardFrame
    
    %% InventoryFrame --|> tk.Frame : inherits
    %% WarehouseFrame --|> tk.Frame : inherits
    %% OrderFrame --|> tk.Frame : inherits
    %% DashboardFrame --|> tk.Frame : inherits
    ...
    %% Aggregation (Manager holds Products)
    ...
    %% Composition (Warehouse owns Locations)
    Warehouse *-- StorageLocation
    %% Dependency (Engine uses Manager and Warehouse logic)
    ...
    %% Orders use Products and Logger
